...
\begin{itemize}
\item {\bf Distributed Provenance Compression} \hfill Sep 2015 -- Jun 2016\\ 
  Designed and implemented a framework for compressing network
  ...
  concrete copy for provenance trees in the same equivalence
  class. Equivalent provenance trees can be efficiently identified at
  compile time through static analysis of the provenance maintenance
  code, which is specified in NDLog (Network Datalog). At runtime, the
  provenance trees are compressed based on the evalution results of
  the static analysis.
...
\item {\bf Verification of Declarative Networking Programs} \hfill Jun
  2014 -- May 2015\\ 
  Designed and implemented a framework for
  ...
  into dependency graph -- a data structure especially designed for
  static analysis of declarative programs. The speficied safety
  properties are checked against the dependency graph to determine if
  any system execution trace would violate the property. We developed
  a prototype to verify properties of SDN applications, and found
  several bugs out of the applications.


(in collaboration with Prof. Boon Thau Loo, Dr. Limin Jia and Prof. Wenchao Zhou)
\item {\bf Towards a Secure and Verifiable Future Internet} \hfill Sep 2011 -- May 2014\\
Developed a unified framework for combining implementation and formal
